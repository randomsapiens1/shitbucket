    ...
    <Section label="links & inspo">
      {(links || []).map(l => (
        <div key={l.id} className="flex justify-between items-center py-2.5 border-b border-black/10 last:border-0">
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="text-[#FF6A00] text-[13px] font-bold no-underline break-all flex-1 hover:underline"
          >
            {l.label || l.url}
          </a>
      ...
      {!show ? (
        ...
        <div className="flex flex-col gap-2 mt-2">
          <input
            className="w-full bg-[#FFF8EE] border-2 border-black/20 focus:border-black rounded-xl px-3 py-2.5 text-black font-bold text-[13px] outline-none transition placeholder:text-black/30"
            placeholder="url"
            value={url}
