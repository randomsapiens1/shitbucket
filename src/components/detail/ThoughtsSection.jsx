...
export default function ThoughtsSection({ thoughts, newThought, setNewThought, onAdd, onRemove, onUpdate }) {
  return (
    <Section label="thoughts">
      {(thoughts || []).map(t => (
        <div key={t.id} className="bg-[#FFF8EE] border border-black/20 rounded-xl px-4 py-3 mb-2 relative group focus-within:border-black transition-colors">
          <AutoResizeTextarea
            className="w-full bg-transparent text-[calc((13/12)*var(--base-font-size))] font-bold text-black leading-relaxed pr-6 outline-none border-none"
            value={t.text}
